...
[04_regression]  OLS 다중회귀 + Logistic 매개효과 분석 (Bootstrap 5,000회)
...
### 1. 파일명 및 목적
FPI가 별점과 폐업여부에 미치는 영향을 검증하고, Bootstrap 매개효과 분석으로 `FPI → 별점 → 폐업`의 간접 경로를 규명.
...
**STEP 4-4 — Bootstrap 매개효과 분석 (5,000회)**

```python
np.random.seed(42)
N_BOOTSTRAP = 5000
a = model_a.params['fpi_300m']   # FPI → 별점: -0.0271
b = model_b.params['stars']      # 별점 → 폐업: -0.2997
indirect = a * b                 # 간접효과: +0.0081
```

3,709회 수렴 성공 (수렴률 74.2%)
...
**완전 매개(Full Mediation)**: `FPI → 별점 하락 → 폐업 확률 상승` 경로 통계적으로 유의.

### 5. 개선 아이디어
- Bootstrap 수렴 실패율 25.8% → neighborhood 더미 축소 또는 Ridge 로지스틱 도입
- SOBEL TEST 병행으로 매개효과 유의성 재검증
- 패널 데이터 구축 후 고정효과 모델로 역인과 가능성 통제
...
2. **폐업 경로는 직접적이지 않음**: `FPI → 별점 하락 → 폐업` 간접 매개 경로가 통계적으로 유의
