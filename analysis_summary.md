...
> 보고서 담당자 및 팀원용 요약 문서
> 초기 분석 계획 대비 실제 진행 내용과 변경 이유를 정리한다.
...
PART 2 — 업종 특화 심화 분석 (버거+샌드위치+패스트푸드)
...
#### STEP 3~4. 텍스트 전처리 및 감성점수 산출 (민서 담당)
...
>
> 이를 해결하기 위해 **PART 2에서 버거+샌드위치+패스트푸드 업종으로 범위를 좁혀
> 동일한 분석을 수행**하였다.
...
### 업종 정의 근거
버거 독립 브랜드의 경쟁자는 버거 프랜차이즈만이 아니다.
소비자가 "간편한 한 끼"를 선택할 때 버거, 샌드위치, 패스트푸드는
동일한 Consideration Set 안에 있는 대체재다.
...
- 영업 중 독립 브랜드: 535개
- **주목**: 팀플(18.3%) 대비 프랜차이즈 비율 50.9%로 압도적으로 높음

#### STEP 2. 감성점수 산출
- PART 1(민서)과 동일한 방식 적용
- **VADER 상관계수: 0.792** (팀플 0.724보다 높음)
...
notebooks/
├── PART1_restaurants/
│   ├── 01_data_setup.ipynb       ← 데이터 준비 (선우)
│   ├── 02_dtm_tfidf.ipynb        ← DTM/TF-IDF (민서)
│   ├── 03_sentiment.ipynb        ← 감성점수 (민서)
│   ├── 04_fpi.ipynb              ← FPI 산출 (선우)
│   ├── 05_regression.ipynb       ← 회귀분석 (선우)
│   ├── 06_text_analysis.ipynb    ← 텍스트 분석 (민경, 한계 명시)
│   ├── 07_survival.ipynb         ← 생존 브랜드 (선우, 한계 명시)
│   └── 08_fpi_map.ipynb          ← FPI 지도 (선우)
│
└── PART2_burger_qsr/             ← 업종 특화 심화 분석 (선우)
    ├── 01_data_prep.ipynb
    ├── 02_sentiment.ipynb
    ├── 03_fpi.ipynb
    ├── 04_regression.ipynb
